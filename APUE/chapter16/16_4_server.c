...
initServer(int type, int protocol, struct addrinfo *info, int qLen)
{
	...
	if (type == SOCK_STREAM || type == SOCK_SEQPACKET) {
		if (-1 == listen(fd, qLen)) {
			syslog(LOG_ERR, "listren faield! %s", strerror(errno));
			return -1;
		}
	}
...
serve(int fd)
{
	...
	while (1) {
		...
		if (NULL == (fp = popen("ps -ef | wc -l", "r"))) {
			sprintf(buf, "popen failed! %s", strerror(errno));
...
main(int argc, char const *argv[])
{
	...
	if (0 != (err = getaddrinfo(host, "processCount", &hint, &ailist))) {
		printf("getaddrinfo faield! %s\n", gai_strerror(err));
		return -1;
	}

	for (aip = ailist; aip != NULL; aip = ailist->ai_next) {
		if (0 < (fd = initServer(SOCK_STREAM, 0, aip, 0))) {
			serve(fd);
			exit(0);
		}
	}
